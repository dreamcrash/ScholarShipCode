...
void run(int size, int validation, int numThreads)
{
    ...
    if(buildTestData(data) == 0) return ; // problems
...
int buildTestData(Ideatest *data){
    ...
    forceFirstTouchMemoryPolicy(data);
...
void Do (Ideatest *data)
{
    #pragma omp parallel
    {
        cipher_idea(data->plain1, data->crypt1, data->Z,    data->array_rows);
        cipher_idea(data->crypt1, data->plain2, data->DK,   data->array_rows);
    }
}
...
void forceFirstTouchMemoryPolicy(Ideatest *data)
{
    const int array_row = data->array_rows;
