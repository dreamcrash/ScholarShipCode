...
void run(const int size, const int validation){
    ...
    const int datasizes[]   = {500, 1000, 2000, 8000, 16000}; 
    int info;
    Linpack linpack;
    linpack.size = size;
    ...
    linpack.a = processMatrixAllocation(n, num_process, rank);
    
    if(!JGFinitialise(&linpack, n, rank)) return ; // return if problems
    
    // Send the values from matrix a from the master to the slaves
    sendAndRecvSubMatrixCycleManner (n, n+1, linpack.a);
    
    MPI_Barrier(MPI_COMM_WORLD);
    const double start = omp_get_wtime();
    
    info = dgefa(n, linpack.a, linpack.ipvt, rank, num_process);
    if(rank == 0)
    {
        dgesl(n, linpack.a, linpack.ipvt, linpack.b);
    }
...
double** processMatrixAllocation(const int n, int num_process, int rank){
 ...
 int JGFinitialise(Linpack *linpack, const int n, int rank){
...
int dgefa(const int n, double **a, int ipvt[n], int rank, int total_process){
    ...
    if (nm1 >=  0) 
    {
        for (int k = 0, worker = 0, kp1 = 1; k < nm1; k++, kp1++, worker++)
        {
            ...
            {
		double *col_k = a[k]; 
		int l = tmp_l[0] = idamax(n-k,col_k,k,1) + k; // find l = pivot index
