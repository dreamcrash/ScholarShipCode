...
MD *newMD(int size,int *datasizes, int iterations, int numThreads){
    ...
    md->processID = getProcessId();
    md->totalProcess = numberProcess();
...
void cicleForces(MD *md, Reduction **thrPrivateData){
    ...
    calculate_force (md, thrPrivateData);
    reduceForces    (md);
    reduceStaticVariable(md);
}
...
void runMD(MD *md){
    ...
    Reduction **thrPrivateData = createReduction(md->particlesSOA, md->mdsize, md->numThreads);
    omp_set_dynamic(0);                   // Explicitly disable dynamic teams
    omp_set_num_threads(md->numThreads);  // Use N threads for 
    
    for(int move = 0; move < md->movemx; ++move)
    {
        cicleDoMove                  (md);
        cicleForces                  (md, thrPrivateData);
        cicleMkekin                  (md);
        cicleVelavg                  (md);
