...
/** Force calculation */
void calculate_force(MD *md, Reduction **thrPrivateData){
    
    #pragma omp parallel
    {
        int numberParticles = md->mdsize;
        int idThread = omp_get_thread_num();
        initThreadRelatedVariables(thrPrivateData, idThread, numberParticles);
        
        #pragma omp for schedule (dynamic,1)
        for(int i = 0; i < numberParticles; i++)
        {
            force3Law(md, md->particlesSOA, i, thrPrivateData[idThread]);
        }
     } 
     reduceThreadRelatedVariables(md, thrPrivateData);
 }
...
void force3Law(MD *md, Particles *p, int particleA, Reduction *thrPrivateData){
        ...
        for (int particleB = particleA + 1; particleB < numParticles; particleB++)
	{
	    ...
	    if(rd <= radius)
	    {
	        ...
	        const double r148 = rrd7 - 0.5 * rrd4;
                ...
                thrPrivateData->fx[particleB] -= tmpFx;
                thrPrivateData->fy[particleB] -= tmpFy;
                thrPrivateData->fz[particleB] -= tmpFz;  
        ...
        /** Force Calculation **/
        thrPrivateData->fx[particleA] += fxAcc;
        thrPrivateData->fy[particleA] += fyAcc;
        thrPrivateData->fz[particleA] += fzAcc;
        
        /** MD control variables actualization */
        thrPrivateData->epot         += epot;
        thrPrivateData->vir          += vir;
        thrPrivateData->interactions += interactions;   
}
