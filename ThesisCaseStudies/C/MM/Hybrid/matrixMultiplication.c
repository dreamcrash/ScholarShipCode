...
void matrixMuliplication(   const int version, const int processID, const int numProcesses,
                            const int maxRowA, const int maxColA, const int maxRowB, const int maxColB,
                            double A[maxRowA][maxColA], double B[maxColA][maxColB], double C[maxRowA][maxColB]){
    
    int maxRowC = maxRowA, maxColC = maxColB;
    
    MPI_Barrier(MPI_COMM_WORLD);
    const double begin = MPI_Wtime();
    
    /*   
     *  This function takes advantage of the fact that the matrix in continously 
    *   allocated in memory, sending a single msg with the entire matrix.
    */
    MPI_Allreduce(MPI_IN_PLACE, B, maxRowB * maxColB, MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
    ...
    {
        masterSendSubMatricesToSlaves(maxRowA, maxColA, A, tilei, processID, numProcesses);
        
        const int processChunk = (processID == 0) ? tilei * numProcesses : tilei;     // Calculating process indices accordingly
        optimizedMM(maxRowA, maxColB, maxColA, A, B, C, processChunk);

        masterCollectsSubMatrix(maxRowC, maxColC, C, tilei, processID, numProcesses);
    }
...
void optimizedMM(const int maxRowA, const int maxColB, const int maxColA, 
    ...
    #pragma omp parallel
    {
        ...
        for(int jj = 0; jj < maxColC; jj += tilej)     
        {
            ...
            #pragma omp for schedule(dynamic)
            for(int ii = 0; ii < maxRowC; ii += processChunk)
            {
                initBlock       (cc);
